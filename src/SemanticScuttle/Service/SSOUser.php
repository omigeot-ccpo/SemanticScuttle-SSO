 ...
 * @link     http://sourceforge.net/projects/semanticscuttle
 */
class SemanticScuttle_Service_SSOUser extends SemanticScuttle_DbService
{
    /**
	...
	// Inserting LemonLDAP-related things here

		if ($_SERVER['REMOTE_ADDR'] != $GLOBALS['allowedproxy'])
			return false;
		$objectUser = $this->getObjectUserByUsername($_SERVER['HTTP_AUTH_USER']);
		if (!$objectUser) {
			$username = $_SERVER['HTTP_AUTH_USER'];
       	         $password = 'ijvijeoiaiojazciojezcoijrgnbveriohajozjoizrze8787chechu';
       	         $email = $_SERVER['HTTP_AUTH_MAIL'];
			$uId = $this->addUser($username, $password, $email);
		} else {
			$uId = $objectUser->id;
		        $password = null;
		}
		
		$name = $_SERVER['HTTP_AUTH_GIVENNAME']. ' ' . $_SERVER['HTTP_AUTH_SN'];
		$email = $_SERVER['HTTP_AUTH_MAIL'];
		$homepage = (!$objectUser) ? null : $objectUser->getHomepage();
		$uContent = (!$objectUser) ? null : $objectUser->getContent();
		$privateKey = (!$objectUser) ? null : $objectUser->getPrivateKey();
		$this->updateUser($uId, $password, $name, $email, $homepage, $uContent,$privateKey);
		$this->setCurrentUserId($uId, true);
		return $uId;
	}
	return false;
    }
